  ...
  return function(data) {
    ...
    data.content = content.replace(regex, (match, yamlStr) => {
      try {
        ...
        const widthVal = width ? (/^\d+$/.test(width) ? width + 'px' : width) : null;
        const heightVal = height ? (/^\d+$/.test(height) ? height + 'px' : height) : null;

        let containerStyle = '';
        let mediaStyle = ''; // 专门给 img 和 video 使用的样式
        if (widthVal && heightVal) {
          // 宽高都设
          containerStyle = `width: ${widthVal}; height: ${heightVal};`;
          mediaStyle = `width: 100%; height: 100%; object-fit: cover;`;
        } else if (widthVal) {
          // 只设宽
          containerStyle = `width: ${widthVal};`;
          mediaStyle = `width: 100%; height: auto;`;
        } else if (heightVal) {
          // 只设高
          containerStyle = `height: ${heightVal}; width: fit-content;`;
          mediaStyle = `height: ${heightVal}; width: auto; max-width: 100%; object-fit: contain;`;
        }else{
          containerStyle = `width: fit-content; max-width: 100%;`;
          mediaStyle = `max-width: 100%; height: auto;`;
        }

        let html = `<div class="live-photo-container" ${containerStyle ? `style="${containerStyle}"` : ''}>`;

        html += `<img src="${processedImageUrl || 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7'}" alt="${altText}" class="live-photo-static" style="${!processedImageUrl ? 'display:none; ' : ''}${mediaStyle} opacity: 1;">`;

        html += `<video muted playsinline autoplay ${loop ? 'loop' : ''} preload="${config.preload || 'auto'}" class="live-photo-video" src="${processedVideoUrl}" style="${mediaStyle}"></video>`;
        ...
        html += `<div class="live-error-placeholder">${failSvg}</div>`;
