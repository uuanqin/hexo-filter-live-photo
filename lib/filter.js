...
module.exports.filter_logic = function (hexo, config) {
  ...
  return function (data) {
    ...
    const {protectedContent, restoreCt} = protectionTool(
      tempContent,
      REGEX_CODEBLOCK,
      (match, _1, _2, lang) => {
        const l = lang.trim().toLowerCase();
        return l === 'live-photo' || l === 'livephoto';
      });

    tempContent = protectedContent.replace(REGEX_LIVE_PHOTO, replaceLivePhotoBlock(data));
    data.content = restoreCt(tempContent);
...
function protectionTool(content, regex, skipCondition) {
  ...
  const protect_symbol = `_Pr0tECt_${seed}_hxFTlp_`;
